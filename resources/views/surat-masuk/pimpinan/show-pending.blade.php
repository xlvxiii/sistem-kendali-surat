...
@section('content')
    <div id="main">
            ...
            <div class="page-title">
                <div class="row">
                    <div class="col-12 col-md-6 order-md-1 order-last">
                        <h3>{{ __($title) }}</h3>
                        <p class="text-subtitle text-muted">{{ __('Anda login sebagai pimpinan!') }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="page-content">
            <a href="{{ url('suratMasukPimpinan/pending') }}" class="btn btn-primary mb-3">
                <i class="fas fa-chevron-left"></i> {{ __('Kembali') }}
            </a>
            @empty($suratMasuk->DisposisiPertama)
            ...
            <div class="modal fade" id="disposisiModal" tabindex="-1" aria-labelledby="disposisiModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <form method="post" action="/suratMasukPimpinan/pending/{{ $suratMasuk->id }}">
                            @csrf
                            @method('put')
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    <label for="divisi_id" class="col-form-label">Ditujukan:</label>
                                    <select class="form-select" id="divisi_id" name="divisi_id" required>
                                        <option value="" default selected disabled>--Pilih divisi--</option>
                                ...
                                <div class="mb-3">
                                    <label for="jenis_disposisi" class="col-form-label">Instruksi:</label>
                                    <select class="form-select" id="jenis_disposisi" name="jenis_disposisi" required>
                                        <option value="" default selected disabled>--Pilih instruksi--</option>
                                        @foreach ($jenis_disposisis as $jenis_disposisi)
                                            <option value="{{ $jenis_disposisi->jenis_disposisi }}">{{ ucwords($jenis_disposisi->jenis_disposisi) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="mb-3">
                                        ...
                                        <div class="row">
                                            <table class="display table table-borderless table-hover table-striped fs-6">
                                                ...
                                                <tr>
                                                    <td>{{ 'Ditujukan' }}</td>
                                                    <td>{{ ':' }}</td>
                                                    <td>{{ ucwords($suratMasuk->DisposisiPertama->tujuan) }}</td>
                                                </tr>
                                                <tr>
                                                    <td>{{ 'Tanggal' }}</td>
                                                    <td>{{ ':' }}</td>
