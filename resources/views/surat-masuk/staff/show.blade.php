...
@section('content')
    <div id="main">
            ...
            <div class="page-title">
                <div class="row">
                    <div class="col-12 col-md-6 order-md-1 order-last">
                        <h3>{{ __($title) }}</h3>
                        <p class="text-subtitle text-muted">{{ __('Anda login sebagai staff!') }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="page-content">
            <a href="{{ url('suratMasukStaff') }}" class="btn btn-primary mb-3">
                <i class="fas fa-chevron-left"></i> {{ __('Kembali') }}
            </a>
            @if ($suratMasuk->status != 'Selesai')
                <button type="button" class="btn btn-success mb-3" name="konfirmasi-button" id="konfirmasi-button">
                    <i class="bi bi-bookmark-check-fill"></i> {{ __('Konfirmasi') }}
                </button>

                <form action="/suratMasukStaff/{{ $suratMasuk->id }}" method="post" id="update-form">
                    @csrf
                    @method('put')
    ...
    <script>
        document.getElementById("konfirmasi-button").addEventListener("click", (e) => {
            Swal.fire({
                icon: "question",
